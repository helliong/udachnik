...
if(isset($_POST['submit'])){
    ...
    if (count($errors) === 0) {
        ...
        $prepared = $db->prepare('UPDATE posts SET title=:title, intro=:intro,
        content=:content, image_path=:image_path WHERE id=:post_id');
        $prepared->execute([
            'title' => $title,
            'intro' => $intro,
            'content' => $content,
...
<main> 
    ...
    <form action="edit.php?id=<?= $post['id']; ?>" method="post" enctype="multipart/form-data">
            <div>
                ...
                <input type="text" name="title" id="post-title" value="<?=$title ?? $post['title']?>">
                <?= $errors['title']; ?>
            </div>
            <div>
                <label for="post-intro">Анонс поста:</label>
                <input type="text" name="intro" id="post-intro" value="<?=$intro ?? $post['intro']?>">
                <?= $errors['intro']; ?>
            </div>
            <div>
